...
function SectorBadge({ label, estado }: { label: string; estado: string }) {
  ...
  return <span style={{ background: color, color: '#fff', borderRadius: 6, padding: '2px 8px', fontSize: 11, fontWeight: 600, marginRight: 4, display: 'inline-block' }}>{label}: {estado || 'Pendiente'}</span>
}
...
export function CasoCard({ caso, onUpdate, sector, showDelete }: {
  caso: Caso; onUpdate: () => void
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 12, marginBottom: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', opacity: isCerrado ? 0.55 : 1, borderLeft: `4px solid ${bg}`, overflow: 'hidden' }}>
      <div style={{ padding: '14px 16px' }}>
        ...
        <button onClick={() => { setExpanded(!expanded); if (!expanded) loadActs() }} style={{ marginTop: 10, background: 'transparent', border: 'none', color: '#007271', fontSize: 12, cursor: 'pointer', fontWeight: 600, padding: 0 }}>
          {expanded ? '▲ Ocultar hilo' : `▼ Ver actualizaciones${acts.length > 0 ? ` (${acts.length})` : ''}`}
        </button>
      </div>
          ...
          <div style={{ paddingTop: 12 }}>
            ...
            {!isCerrado && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, borderTop: acts.length > 0 ? '1px solid #F3F4F6' : 'none', paddingTop: acts.length > 0 ? 12 : 0 }}>
                ...
                <button onClick={handleAgregar} style={{ background: '#007271', color: '#fff', border: 'none', borderRadius: 6, padding: '8px 18px', fontSize: 12, cursor: 'pointer', fontWeight: 600, alignSelf: 'flex-start' }}>
                  Agregar
                </button>
              </div>
            )}
