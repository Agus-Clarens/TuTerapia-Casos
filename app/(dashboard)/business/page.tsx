...
export default function Page() {
  ...
  async function load() {
    ...
    if (data) setCasos(data.filter((c: Caso) => c.area === 'Business'))
    setLoading(false)
  }
  ...
  return (
    <div style={{padding: 24}}>
      <h1 style={{fontSize: 22, fontWeight: 700, color: '#264534', marginBottom: 16}}>Business</h1>
      {loading ? <p>Cargando...</p> : casos.length === 0
        ? <p style={{color:'#9CA3AF'}}>No hay casos.</p>
        : [...casos].sort((a,b) => ord(a.estado)-ord(b.estado)).map(c =>
            <CasoCard key={c.id} caso={c} onUpdate={load} sector="business" />
          )
      }
    </div>
  )
