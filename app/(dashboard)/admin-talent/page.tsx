...
const ord = (e: string) => ({'Nuevo':0,'En curso':1,'Cerrado':3} as any)[e] ?? 2
...
export default function AdminTalentPage() {
  ...
  async function load() {
    ...
    if (data) setCasos(data.filter((c: any) => c.area === 'Admin+Talent'))
    setLoading(false)
  }
  ...
  return (
    <div style={{padding: 24}}>
      <h1 style={{fontSize: 22, fontWeight: 700, color: '#264534', marginBottom: 16}}>Admin + Talent</h1>
      {loading ? <p>Cargando...</p> : casos.length === 0
        ? <p style={{color:'#9CA3AF'}}>No hay casos.</p>
        : [...casos].sort((a,b) => ord(a.estado)-ord(b.estado)).map((c: any) =>
            <CasoCard key={c.id} caso={c} onUpdate={load} sector="todos" />
          )
      }
    </div>
  )
