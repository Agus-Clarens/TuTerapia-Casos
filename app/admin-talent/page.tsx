...
export default function AdminTalentPage() {
  ...
  const fetchCasos = useCallback(async () => {
    ...
    if (filtroEstado) qb = qb.eq('estado', filtroEstado)
  ...
  async function cerrarAdmin(caso: Caso) {
    setClosing(caso.id! + '_admin')
    const updates: Record<string, unknown> = { cerrado_admin: true }
    if (caso.cerrado_talent) updates.estado = 'Cerrado'
  ...
  async function cerrarTalent(caso: Caso) {
    setClosing(caso.id! + '_talent')
    const updates: Record<string, unknown> = { cerrado_talent: true }
    if (caso.cerrado_admin) updates.estado = 'Cerrado'
    await supabase.from('casos').update(updates).eq('id', caso.id!)
    await fetchCasos()
    setClosing(null)
  }
    ...
    <div className="p-8">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-6 flex gap-3">
        ...
        <select value={filtroEstado} onChange={e => setFiltroEstado(e.target.value)}>
          ...
          {['Nuevo', 'En curso', 'Requiere atención', 'Cerrado'].map(e => <option key={e} value={e}>{e}</option>)}
        </select>
        {(filtroEstado || filtroPais || search) && (
                      ...
                      <div className="flex flex-col gap-1">
                        {!caso.cerrado_admin ? (
                          <button
                            ...
                            disabled={closing === caso.id! + '_admin'}
                            className="px-2.5 py-1 text-xs bg-white text-blue-600 rounded-lg border border-blue-200 hover:bg-blue-50 transition-colors disabled:opacity-50"
                          >
                            {closing === caso.id! + '_admin' ? '…' : 'Cerrar Admin'}
                          </button>
                        ) : (
                          <span className="px-2 py-0.5 text-xs bg-green-50 text-green-700 rounded-lg border border-green-200 text-center">Admin ✓</span>
                        )}
                        {!caso.cerrado_talent ? (
                          <button
                            onClick={() => cerrarTalent(caso)}
                            disabled={closing === caso.id! + '_talent'}
                            className="px-2.5 py-1 text-xs bg-white text-purple-600 rounded-lg border border-purple-200 hover:bg-purple-50 transition-colors disabled:opacity-50"
                          >
                            {closing === caso.id! + '_talent' ? '…' : 'Cerrar Talent'}
                          </button>
                        ) : (
                          <span className="px-2 py-0.5 text-xs bg-green-50 text-green-700 rounded-lg border border-green-200 text-center">Talent ✓</span>
                        )}
                      </div>
                    )}
                  </div>
