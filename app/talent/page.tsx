...
export default function TalentPage() {
  ...
  const fetchCasos = useCallback(async () => {
    ...
    const [r1, r2] = await Promise.all([
      applyFilters(supabase.from('casos').select('*').in('area', ['Talent', 'Admin+Talent'])),
      applyFilters(supabase.from('casos').select('*').in('cargado_por', CREADORES_POR_AREA.Talent)),
    ])
  ...
  async function cerrarTalent(caso: Caso) {
    setClosing(caso.id!)
    const updates: Record<string, unknown> = { cerrado_talent: true }
    if (caso.area === 'Talent' || (caso.area === 'Admin+Talent' && caso.cerrado_admin)) {
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-2xl font-bold text-verde-oscuro">Talent</h1>
          </div>
          <p className="text-gray-400 text-sm">{casos.length} casos · Talent · Admin+Talent · creados por Orne y Caro</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {Object.entries(counts).map(([est, n]) => (
          <button key={est} onClick={() => setFiltroEstado(f => f === est ? '' : est)}
            className={`rounded-xl border p-3 text-left transition-all ${filtroEstado === est ? 'border-purple-400 bg-purple-50' : 'bg-white border-gray-100 hover:border-gray-200'}`}>
                ...
                <div className="px-5 py-4 flex items-start gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1.5">
                      ...
                      {caso.area !== 'Talent' && (
                        <span className={`px-2 py-0.5 text-xs rounded-full border font-medium ${caso.area === 'Admin+Talent' ? 'bg-orange-50 text-orange-700 border-orange-200' : caso.area === 'CX' ? 'bg-verde-medio/10 text-verde-oscuro border-verde-medio/30' : 'bg-blue-50 text-blue-700 border-blue-200'}`}>
                  ...
                  <div className="flex flex-col items-end gap-2 flex-shrink-0">
                    ...
                    {caso.estado !== 'Cerrado' && !caso.cerrado_talent && (
                      <button
                        onClick={() => cerrarTalent(caso)}
                      ...
                      >
                        {closing === caso.id ? '…' : 'Cerrar Talent'}
                      </button>
                    )}
                    {caso.cerrado_talent && caso.area === 'Admin+Talent' && caso.estado !== 'Cerrado' && (
                      <span className="px-2 py-0.5 text-xs bg-green-50 text-green-700 rounded-lg border border-green-200">Talent ✓</span>
                    )}
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-2">
                    {isNew && !isExpanded ? (
                      ...
                      <div className="flex items-center gap-1.5 text-xs text-gray-400">
                        ...
                        {latestUpdates[caso.id!] ? <span className="font-medium text-purple-600">Ver hilo</span> : 'Sin actualizaciones'}
                      </div>
                    )}
                  </div>
