...
export default function TalentPage() {
  ...
  const fetchCasos = useCallback(async () => {
    setLoading(true)
    let query = supabase.from('casos').select('*').in('area', ['Talent', 'Admin+Talent']).order('created_at', { ascending: false })
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm">{casos.length} casos · Talent y Admin + Talent</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400 bg-white rounded-2xl border border-gray-100">Sin casos Talent</div>
      ) : (
              ...
              <div key={caso.id} className={`bg-white rounded-2xl border shadow-sm overflow-hidden ${caso.estado_general === 'Cerrado' ? 'opacity-60 border-gray-100' : 'border-gray-100 hover:border-gray-200'}`}>
                ...
                >
                  <div className="flex items-center gap-2 text-xs text-gray-400">
                    ...
                    {updCount > 0
                      ? <span className="font-semibold text-purple-600">{updCount} actualización{updCount > 1 ? 'es' : ''}</span>
                      : 'Sin actualizaciones'}
                  </div>
