...
export default function CXPage() {
  ...
  const fetchCasos = useCallback(async () => {
    setLoading(true)
    let query = supabase.from('casos').select('*').eq('area', 'CX').order('created_at', { ascending: false })
    ...
    if (!error && data) {
      ...
      result.forEach(c => { initial[c.id!] = c.observaciones || '' })
      setActions(initial)
    }
    setLoading(false)
  ...
  async function tomarCaso(id: string) {
    setSaving(id)
    await supabase.from('casos').update({ estado_general: 'En curso' }).eq('id', id)
    await fetchCasos()
    setSaving(null)
  }
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm">{casos.length} casos de experiencia al paciente</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {Object.entries(counts).map(([est, n]) => (
          <button
            key={est}
            onClick={() => setFiltroEstado(f => f === est ? '' : est)}
            className={`rounded-xl border p-4 text-left transition-all ${filtroEstado === est ? 'border-verde-oscuro bg-verde-oscuro/5' : 'bg-white border-gray-100 hover:border-gray-200'}`}
          >
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400 bg-white rounded-2xl border border-gray-100">Sin casos CX</div>
      ) : (
        <div className="space-y-3">
          {casos.map(caso => {
            ...
            return (
              <div key={caso.id} className={`bg-white rounded-2xl border shadow-sm overflow-hidden transition-all ${isClosed ? 'opacity-60 border-gray-100' : 'border-gray-100 hover:border-gray-200'}`}>
                {/* Header */}
                <div className="px-5 py-4 flex items-start gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2 flex flex-wrap gap-3 text-xs text-gray-500">
                      <span>
                        <span className="text-gray-400">Pac:</span> <span className="font-medium text-gray-700">{caso.pac_nombre}</span>
                        {caso.pac_mail && <span className="text-gray-400 ml-1">· {caso.pac_mail}</span>}
                      </span>
                      {caso.psi_nombre && (
                        <span><span className="text-gray-400">Psi:</span> {caso.psi_nombre}</span>
                      )}
                    </div>
                  </div>
                ...
                {!isClosed && (
                  <div className="px-5 pb-4 border-t border-gray-50 pt-3">
                    {(isEnCurso || isResuelto) && (
                      <div className="mb-3">
                        <label className="text-xs font-medium text-gray-500 mb-1.5 block">Acción tomada</label>
                        <textarea
                          value={actions[caso.id!] ?? ''}
                          onChange={e => setActions(a => ({ ...a, [caso.id!]: e.target.value }))}
                          disabled={isResuelto || saving === caso.id}
                          rows={2}
                          placeholder="Describí qué se hizo para resolver este caso..."
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isResuelto && caso.observaciones && (
                        <span className="text-xs text-gray-500 italic">&ldquo;{caso.observaciones}&rdquo;</span>
                      )}
                    </div>
                  </div>
                )}
